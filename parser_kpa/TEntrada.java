...
public class TEntrada {
    ...
    String obtener_parte_codigo(String pnombre){
        int inicio,fin,desdehaciaatras;
        inicio = cad.toLowerCase().replace(" ","").indexOf("algoritmoa"+pnombre.toLowerCase());
        if(inicio<0)
            inicio = cad.toLowerCase().replace(" ","").indexOf("ekintza"+pnombre.toLowerCase());
        inicio = cad.toLowerCase().indexOf("hasiera",inicio)+7;
        desdehaciaatras = cad.toLowerCase().indexOf("ekintza",inicio);
        if(desdehaciaatras>0)
            fin = cad.toLowerCase().lastIndexOf("amaia",desdehaciaatras);
        else
            fin = cad.toLowerCase().lastIndexOf("amaia");
/*        System.out.println("Buscando "+pnombre+" en "+cad);
        System.out.println("inicio="+String.valueOf(inicio));
        System.out.println("fin="+String.valueOf(fin));
        System.out.println("desdehaciaatras="+String.valueOf(desdehaciaatras));
*/        return(";"+cad.substring(inicio,fin).replace("\n","\n;"));
    }
    TElemEnt leersigterm(TElemEnt p_ant, TTablaSimbolos ts) {
        ...
        String[] pal_clave = "algoritmoa,aldagaiak,hasiera,amaia,irakur,idatz,aukera,amaukera,errepika,egin,harik-eta,baldin,orduan,hemendik-hasita,heldu-arte,bitartean,ambitartean,deitu,karakterea,osoa,boolearra,taula,osagai,bestela,ekintza,karakteretako,osotako,itzuli".split(",");
        ...
        while (ch == ' ' || ch == Character.LINE_SEPARATOR || ch == '\t' || ch =='\n') {
            indice++;
            if (indice == cad.length()) { // se ha llegado al final, devolver $
                UdLex.token = new String("$");
                UdLex.atributo = -1;
        ...
        if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z')) {
            ...
            for (int x = 0; x < pal_clave.length; x++) // ver si el identificador es una palabra clave, no se tiene en cuenta mayusculas/minusculas
                if (lexema.toString().compareToIgnoreCase(pal_clave[x]) == 0) {
                    UdLex.token = new String(pal_clave[x]);
                    UdLex.atributo = -1;
                    return UdLex;
                }
            if (lexema.toString().compareToIgnoreCase("egiazkoa") == 0 || lexema.toString().compareToIgnoreCase("faltsua") == 0){   
            ...
            case '-':
                ...
                UdLex.atributo = ts.insertar("-");
                indice++;
		if(!(p_ant.token.equals("NUM")||p_ant.token.equals("ID"))){
                    ch = cad.charAt(indice);
